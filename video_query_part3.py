# %%writefile video_query_part3.py
import sys
sys.path.append('../resource/lib/publicdata/Code')
...
parser.add_argument("training_set", help="Path to training videos and wav fi`les")
...
if args.f == features[1] or args.f == features[2]:
...
while(cap.isOpened() and cap.get(cv2.CAP_PROP_POS_FRAMES) < (int(args.e)*frame_rate)):
    ...
    if args.f == features[0]:
        ...
        q_colorhist.append(h_colorhist)

        audio_frame = frame_to_audio(frame_nbr, frame_rate, fs, wav_data)
        h_mfcc, mspec, spec = ft.extract_mfcc(audio_frame, fs)
        q_mfcc.append(h_mfcc)
...
def sliding_window(x, w, compare_func):
    ...
    frame = -1 # init frame
    wl = len(w)
    time = np.zeros(len(x) - wl)
    score = np.zeros(len(x) - wl)
    minimum = sys.maxsize
...
for video in video_list:
    ...
    if args.f == features[0]:
        ...
        w = np.array(q_colorhist)
        x = search.get_colorhists_for(video)
        frame, score1 = sliding_window(x,w, euclidean_norm_mean)

        w = np.array(q_mfcc)
        x = search.get_mfccs_for(video)
        #frame, score = sliding_window(x,w, euclidean_norm_mean)
        availableLength= min(x.shape[1],w.shape[1])
        frame, score2 = sliding_window(x[:,:availableLength,:],w[:,:availableLength,:], euclidean_norm_mean)

        score = score1 / np.max(score1) + score2 / np.max(score2)
