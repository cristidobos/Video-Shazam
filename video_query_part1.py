# %%writefile video_query_part1.py
import sys
...
def sliding_window(x, w, compare_func):
    ...
    frame = -1 # init frame
    wl = len(w)
    minimums = [sys.maxsize] * 3 # initialize minimums with very high values
    frames = [-1] * 3 # initialize frames with -1
    for i in range(len(x) - wl):
        diff = compare_func(w, x[i:(i+wl)])
        for j in range(3):
            if diff < minimums[j]:
                minimums.insert(j, diff)
                minimums.pop()
                frames.insert(j, i)
                frames.pop()
                break
    for i in range(3):
        print("Match {}: Frame = {}, Score = {}".format(i+1, frames[i], minimums[i]))
    return frames, minimums
...
for video in video_list:
    ...
    if args.f == features[0]:
        ...
        #print(len(x.shape))
        if len(x.shape) == 1:
            print(f"No frames found in {video}. Skipping...")
            continue
